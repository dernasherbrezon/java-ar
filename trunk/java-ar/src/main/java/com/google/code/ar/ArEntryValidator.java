...
class ArEntryValidator {
    ...
    static void validate(ArEntry entry) {
        ...
        if( entry.getFileModificationTimestamp() < 0 ) {
            throw new IllegalArgumentException("entry file modification timestamp should be positive. got: " + entry.getFileModificationTimestamp());
        }
